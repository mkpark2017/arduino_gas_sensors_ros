...
void init_ADPD(){
  ...
  set_config(RESET_ADDR, 0x00); //Software Reset
  ...
  set_config(0x4B, 2692); // Clock enable
...
void loop(){
  byte val[2];
  Wire.beginTransmission(ADPD_ADDR);
  Wire.write(0x00);          //  The command byte, sets pointer to register with register address
  Wire.endTransmission();
  ...
  Serial.print("Conc: ");
  Serial.println((float) dataB/dataA);
  Serial.println("-------------------------------");
